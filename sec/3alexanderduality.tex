...
In this section we present a duality on the normalized chains of the augmented simplex, and use it to build manageable models of the $r$-fold tensor product of the normalized chain complex of an augmented simplicial object in $\cC$. In Proposition \ref{prop:omegarm} and Lemma \ref{lemma:omegar} we reduce the construction of a connected diagonal to the construction of a map between two resolutions of the cyclic group $\Cyc_r$ satisfying an additional hypothesis. This latter map is constructed in \cref{s:resolutions}.

\subsection{Alexander and Poincar\'e dualities}\label{s:alexander}

Given a simplicial set $X$, let $\chains(X) = \sus{}\cadenas(X)$ be the right suspension of the normalized chains of $X$.\anibal{Does \( \sus{} \) use \( X \)?}

Given a finite simplicial complex \( S \) with \( n \) totally ordered vertices, there is a canonicall inclusion \( S \to \simplex^n \).
By considering the complement of ....

These duality, induces at the level of chain complexes a Poincar\'e duality on the \(  \simplex^n_+ \) ...
