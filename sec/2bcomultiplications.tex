...
\begin{proof} See \cite{may1970general}.    %The operations are linear because $\mu$ is linear and Fermat's little theorem in $\mathbb{F}_r$ guarantees that $(\lambda\cdot y)^ \ot {r} = \lambda\cdot (y)^{ \ot  r}$ for a scalar $\lambda$. That the operations are well-defined follows through the usual argument: if $x$ is a representative of a cohomology class, its coboundary vanishes, and so does the coboundary of $y \ot \ldots \ot  y$, hence it maps cocycles to cocycles. If $x = \delta y$ is a coboundary, then FILL.
...
Every unstable comultiplication yields a stable comultiplication by precomposing $\mu$ with the projection $\Wst(r)\to \W(r)$. A stable comultiplication comes from an unstable comultiplication if it vanishes on $\Wst[i](r) \ot  \ucadenas$ for $i<0$.
...
\subsection{Connected comultiplications}\label{s:connected} Let $\mu$ be a stable comultiplication. A way of guaranteeing the desirable property that $\power^i(x)$ vanishes for $i<0$ is to ask for $\mu$ to vanish on $\Wst[i] \ot  \ucadenas$ for $i>(r-1)n$.
