...
A way of guaranteeing the desirable property that $\power^i(x)$ vanishes for $i<0$ is to ask for $\mu$ to vanish on $$\bigoplus_n\bigoplus_{i>(r-1)n}\Wst[i] \ot \ucadenas[n].$$
...
\begin{figure}
 ...
 \vspace{2cm}
	...
	\caption{The complex $\Wdualaug{r} \ot_\theta \rucadenas$ for $r=3$ and $R=\bF_3$.
		The map $\varphi$ goes from Figure \ref{figure:2} to here, is an epimorphism and its kernel is the smallest chain subcomplex that contains the green part. The power operation $\power^0$ is computed at the corners $\bF_3\ot_\theta C^3_n$.}
	\label{figure:3}
\end{figure}

\begin{definition} 
If $\ucadenas$ is a chain complex with a differential of degree $-1$, we define $\rucadenas$ as the chain complex that in degree $rm$ is equal to $\ucadenas[m]$ and is trivial otherwise. We endow it with the same differential as $\ucadenas$, which now has degree $-r$.
...
If $r$ is odd or $r=2$ and $R=\bF_2$, there is a suspension map $\theta \colon \Waug{r} \to \susp{r-1}\Waug{r}$ defined as
\begin{align*}
	\theta(e_0) &= N(e_{r-1}),
	&
	\theta(e_{i}) &= e_{i+r-1}, \quad i\geq r.	
\end{align*}
...
Define the chain map $\varphi \colon \Wst(r)\ot \ucadenas \to \rWd(r) \hotimes \rucadenas$ as
\[
