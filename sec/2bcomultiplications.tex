...
\[
\dotsb \xra{\eta} \susp{2(1-r)}\W{r} \xra{\eta} \susp{1-r} \W{r} \xra{\eta} \W{r}.
\]
...
%\]
where $\eta \colon \W{r} \to \susp{r-1}\W{r}$ is the \emph{suspension homomorphism} that sends $e_{q}$ to $e_{q+1-r}$. This complex is isomorphic to the desuspension of the complex that computes the Tate homology of $\Cyc_r$, which is part of a homotopy fibre sequence %Cohomology, not homology
\[
...
A way of guaranteeing the desirable property that $\power^i(x)$ vanishes for $i<0$ is to ask for $\mu$ to vanish on
	...
	\]
We will now identify the quotient of $\Wst(r)\otimes \ucadenas$ by this graded submodule when $r$ is odd or $r=2$ and $R=\bF_2$ (see Figures \ref{figure:2}). We then define a connected diagonal as a map from this quotient to $\ucadenas^{\otimes r}$.
...
\begin{figure}{top}
	...
	\caption{The complex $\susp{\bs(r-1)}\Wdualaug{r} \ot \rucadenas$ for $r=3$ and $R=\bF_3$. The map $\varphi$ goes from Figure \ref{figure:2} to here, is an epimorphism and its kernel is the smallest chain subcomplex that contains the green part. The power operation $\power^0$ is computed at the corners $\bF_3\ot C^3_n$.} 
	\label{figure:3}
\end{figure}
