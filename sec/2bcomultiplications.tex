...
{\color{red} Chain complexes in this section are assumed to be non-negatively graded.}
...
\begin{definition}
	...
	\[
	\W{r} \ot \ucadenas \xra{f \ot \id} V(r) \ot \ucadenas \lra \ucadenas^{\ot r}
	\]
	where $V(r)$ is a free resolution of the symmetric group on $r$ letters, $f$ is equivariant with respect to the regular representation of $\Cyc_r$, and the second map is $\Sym_r$-equivariant.
\end{definition}
...
\[
\dotsb \xra{\theta} \susp{2(1-r)}\W{r} \xra{\theta} \susp{1-r} \W{r} \xra{\theta} \W{r}.
\]
...
%\]
where $\theta \colon \susp{1-r}\W{r} \to \W{r}$ is the \emph{suspension homomorphism} that sends $e_{q}$ to $e_{q+1-r}$. This complex is isomorphic to the desuspension of the complex that computes the Tate homology of $\Cyc_r$, which is part of a homotopy fibre sequence %Cohomology, not homology
\[
...
\begin{definition}
	...
	\[
	\dotsb \lra \susp{2(1-r)} \ES_r\overset{\theta}{\lra}\susp{1-r} \ES_r\overset{\theta}{\lra} \ES_r
	\]
	where $\theta$ is the operadic suspension of \cite{berger2004combinatorial} that sends a sequence of permutations $(\sigma_0,\dots,\sigma_q)$ to zero if $(\sigma_0(0),\dots,\sigma_{r-1}(0))$ has repeated elements, and to $\pm (\sigma_{r-1},\sigma_r,\dots,\sigma_q)$ otherwise, where $\pm$ is the sign of the permutation that orders the sequence $(\sigma_0(0),\dots,\sigma_{r-1}(0))$.
\end{definition}

\begin{definition}
	A stable $r$-cyclic diagonal $\mu$ on a chain complex $\ucadenas$ is said to be $\emph{May--Steenrod}$ if it factors as
	\[
...
A stable diagonal comes from an unstable diagonal if it vanishes on $\Wst[i](r) \ot \ucadenas$ for $i<0$ (See \cref{figure:1}).
...
\begin{example}
	As we will see in \cref{s:suspension}, the unstable diagonal for normalized chains in simplicial sets of the previous example, defines a stable diagonal on the normalized chains of simplicial spectra (these are defined in \cite{Gill2020}) which is May--Steenrod.
\end{example}
...
A way of guaranteeing the desirable property that $\power^i(x)$ vanishes for $i<0$ is to ask for $\mu$ to vanish on
	...
	\]
We will now show that if $r$ is odd or $r=2$ and $R=\bF_2$, the quotient of $\Wst(r)\otimes \ucadenas$ by this graded submodule is isomorphic to a tensor product $\Wdualaug{r}\otimes_\theta \rucadenas$ (see Figures \ref{figure:2} and \ref{figure:3}). We then define a connected diagonal as a map from this tensor product to $\ucadenas^{\otimes r}$.
...
\begin{figure}
 ...
 \vspace{2cm}
	...
	\caption{The complex $\susp{\blacksquare(r-1)}\Wdualaug{r} \ot \rucadenas$ for $r=3$ and $R=\bF_3$. The map $\varphi$ goes from Figure \ref{figure:2} to here, is an epimorphism and its kernel is the smallest chain subcomplex that contains the green part. The power operation $\power^0$ is computed at the corners $\bF_3\ot C^3_n$.} 
	\label{figure:3}
\end{figure}
...
R.
\]
Observe that $\rho$ acts ``backwards'' in the dual complex, in the sense that $\rho (e_{i}^\dd) = (\rho^{-1} e_i)^{\dd}$. %The thing is that the correct action is the right action, so that $e_i^\dd \rho=(\rho^{-1}e_i)^\dd$.
The generator of the copy of $R$ in the right will be denoted as $1$ or as $e_0^\dd$.
If $r$ is odd or $r=2$ and $R=\bF_2$, there is a suspension map $\theta \colon \Waug{r} \to \sus{r-1}\Waug{r}$ defined as \martin{This part needs revision, see comments in the tex file}%It should be $\susp{1-r}$ instead of $\susp{r-1}$. Either way, I think you'd rather see %\theta$ as a map $\Waug{r} \to \Waug{r}$ of degree r-1, instead of a suspension map of degree 0. If not, the degrees of the differential of $\Wdualaug{r} \ot \rucadenas$ won't match. fc: I've changed \susp for \sus, was that the issue?
\begin{align*}
	\theta(e_0) &= N(e_{r-1}),
	&
	\theta(e_{i}) &= e_{i+r-1}, \quad i\geq r.	%Where do you send $e_i$ when $0 < i < r$? 
\end{align*}
Let $I$ be the poset of integers. An \emph{ordered sequence of $R$-modules} is a functor $F\colon I\to \Mod{R}$. If $F,G$ are two such functors, then $F\ot G$ denotes the functor obtained by performing the tensor product levelwise. The category $\Ch{R}$ is a subcategory of this category of functors, and the tensor product of a chain complex and any functor is again a chain complex. If we replace the target category $\Mod{R}$ by $\Ch{R}$, we have again that $\Ch{R}$ is a subcategory of the functor category, and that the tensor product of a chain complex and any functor is a bicomplex.

Denote by $\susp{\blacksquare(r-1)}\Wdualaug{r}$ the ordered sequence that in nonnegative takes the form
\[
	...
	\ldots
\]
and vanishes in negative degrees. Recall that $r$ is odd or $R=\mathbb{F}_2$.

\begin{example}
	Let $C$ be a $R$-chain complex, which we view as a $R[\Cyc_r]$-chain complex with trivial $\Cyc_r$ action.  The tensor product $\susp{\blacksquare(r-1)}\Wdualaug{r} \ot C$ is the graded $R[\Cyc_r]$-module
	\[
		\bigoplus_n \susp{n(r-1)}\Wdualaug{r}\ot C_n
	\] 
	with differential
	\[
	...
	\]
\end{example}
There is a functor $Q\colon \asimplexinj\to I$ that sends an ordinal $[m]$ to the number $m$. Thus, any ordered sequence of chain complexes $F$ yields, precomposing with this functor, a simplicial chain complex $Q^*(F)$, and it is easy to see that if $X_\bullet$ is a simplicial $R$-module, then there is a canonical isomorphism
\begin{equation}\label{eq:adj}
	\chains(Q^*(F)\ot X_\bullet)\cong F\ot \chains(X). 
\end{equation}
...
If $r$ is odd or $r=2$ and $R=\bF_2$, a \emph{connected $r$-cyclic diagonal} on a non-negatively graded chain complex $\ucadenas$, or a \textit{connected $r$-diagonal} for short, is a $\Cyc_r$-equivariant chain map
	\[
	\mu \colon \susp{\blacksquare(r-1)}\Wdualaug{r} \ot \rucadenas \lra \ucadenas^{\ot r}.
	\]
\end{definition}
...
Define the chain map $\varphi \colon \Wst(r)\ot \ucadenas \to \susp{\blacksquare(r-1)}\Wdualaug{r} \ot C$ as 
	\[
	\varphi(e_q \ot x) = (-1)^{\binom{n(r-1)-q+1}{2}}e_{q-(r-1)n}^\dd \ot \vec{x}
	\]
if $x$ has degree $n$.
It is an epimorphism with kernel the smallest chain subcomplex that contains $\bigoplus_n\bigoplus_{i>(r-1)n} \Wst[i] \ot \ucadenas[n]$.
As a consequence,
...
\begin{proposition}
	...
	that send a cohomology class $[x]$ in degree $m$ to the cohomology class $[y]$ with
	\[
	y(a) = (-1)^{\binom{ir}{2} + \binom{m}{2}\tilde{r}}(\tilde{r}!)^m(x \ot \overset{r}{\dots} \ot x)(\mu(e_{ri}^\vee \ot a))
	\]
