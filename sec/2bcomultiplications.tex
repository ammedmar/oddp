...
\]
where $\theta_{k} \colon \W(r) \to \sus{-k} \W(r)$ is the \emph{suspension homomorphism} that sends $e_{q}$ to $e_{q+k}$, which only exists if $k$ is even and non-positive. This complex is isomorphic to the desuspension of the complex that computes the Tate homology of $\Cyc_r$, which is part of a homotopy fibre sequence
\[
...
\begin{figure}
 ...
 \vspace{2cm}
	...
	\caption{The complex $\rWd(r) \hotimes \rucadenas$ for $r=3$ and $R=\bF_3$.
		The map $\varphi$ goes from Figure \ref{figure:2} to here, is an epimorphism and its kernel is the smallest chain subcomplex that contains the green part. The power operation $\power^0$ is computed at the corners $\bF_3\hotimes C^3_n$.}
	\label{figure:3}
\end{figure}
...
\begin{definition}
	...
	\[
	\mu \colon \rWd(r) \hotimes \rucadenas \lra \ucadenas^{\ot r}.\anibal{I think this should be a theorem, not a definition. Also, notice that the word chain map is used, but we talked that it is not.}
	\]
\end{definition}

If either $k$ is even \anibal{What is \( k \)? For me, and if is folloed by a then. You mean, let \( k \)...} or $r=2$ and $R=\bF_2$, let $\varphi_k \colon \Wst(r) \to \susp{-k}\rWd(r)$ be the chain map that sends $e_q$ to $(-1)^{\binom{k-q}{2}}e_{k-q}^{\vee}$.
