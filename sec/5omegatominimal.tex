...
The cyclic group $\Cyc_r$ acts on $\partial \asimplex^{r-1}$ by permuting its vertices forwards. Let $\sigma = [0,1,\ldots,r-1]$ be the top simplex of $\asimplex^{r-1}$ and let
\begin{align*}
...
\end{proposition}
\begin{proof}
	Replacing the word $A$ by $\kappa(A)$, we may assume that $A_{j+1} = A_k$. Let $\bar{A} = A_0\barra \ldots\barra A_{j-1}$ and let $\ell$ be the degree of $\bar{A}$ (i.e., the number of entries of $A$). Then %and recall that $\partial A = \sum_{i} (-1)^{i}\diff_iA$, where $\diff_i$ removes the $i$-th entry of $A$. Then
