...
Let $r$ be a positive integer, and let $R$ be a commutative ring with unit. Let $\Mod{R}$ be the category of $R$-modules.
...
The category $\asimplex$ has a strict monoidal product called the \emph{join}, that sends a pair of ordinals $[n],[m]$ to the ordinal $[n+m]$ and a pair of functions $f \colon [n] \to [q]$, $g \colon [m] \to [p]$ to the function $f*g \colon [n+m] \to [q+p]$ defined as $(f*g)(x) = f(x)$ if $x< n$ and $(f*g)(x) = q+g(x-n)$ if $x \geq n$. The unit is the empty ordinal $[0]$.
...
If $X$ is an augmented simplicial object, $X([n])$ is denoted $X_n$ and the morphisms $X(d^U)$ and $X(s^i)$ are denoted $\d_U$ and $s_i$ respectively. If $X$ is an augmented cosimplicial object, $X([n])$ is denoted $X_n$ and the morphisms $X(d^U)$ and $X(s^i)$ are denoted $d^U$ and $s^i$ respectively. \martin{What type of d we want here,$d$ or $\d$? Same for s.}
