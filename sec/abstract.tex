...
\begin{abstract}
    We define a structure on a chain complex that produces power operations in its cohomology with the property that the negative power operations always vanish. We then use this structure to define cochain level operations for augmented simplicial objects and show that they behave nicely with respecto to suspension. These operations remain well-defined in the chain complex of a Kan spectrum. The main tool to build them is a duality on the integral chains of the standard augmented simplex.
\end{abstract}