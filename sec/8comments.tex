...
Below are the times taken to compute the Bockstein ($q=3$) for the model of the Moore space $M(\bZ/3)$ of sagemath and its $m$-fold unreduced suspension (that multiplies the number of faces by $3m$). Below we write $n=m+3$ for the degree of the top simplex in $\susp{m}M(\bZ/3)$, to ease comparison with the previous tables.
\[
...
We then implemented a small simplicial model of $\bC P^3$ found by \cite{datta} with 18 vertices, and considered its $m$th unreduced suspension (that multiplies the number of faces by $3m$) and applied to it an algorithm to compute the first non-trivial operation after the Bockstein ($q=12$). Below we write $n=m+7$ for the degree of the top simplex in $\susp{m}\bC P^3$, to ease comparison with the previous tables.
\[
