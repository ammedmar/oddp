...
\begin{itemize}
	\item[(iii)] If $x\in H(C^*)$ \anibal{is this a typo? I am not clear yet with your notational conventions.} has even degree $m$, then $\power^{m(r-1)}(x)$ is the $r$-fold power of the multiplication of $x$ with itself and all higher power operations of $x$ vanish.
\end{itemize}
...
\end{question}
We assume that $r$ is prime from Section \ref{s:assembly} on, and we assume that $r$ is odd from Section \ref{s:mainresult} on.\anibal{These seems like a randomly placed comment.}
\begin{question}
	Which constructions in this paper can be generalized to composite or even numbers?
\end{question}
\anibal{Are these questions for me or left in the paper?}
