...
\begin{align*}
 ...
 &
	% S\colon \Phi(r)^*&\lra \Sigma^{r-1} \Phi(r)^* \\
...
If $A=(a_0,\ldots,a_k)\in \EC_r$ is a word, let $\sigma(A)$ denote the sign of the permutation that orders the entries of $A$. For a natural number $m$, write $\tilde{m} = \left\lfloor \frac{m}{2}\right\rfloor$.
...
\end{theorem}
%If $r>3$ and $R\neq \bZ_r$, then the $\Cyc_r$-equivariant map $\PSI\colon W^{\leq m(r-1)}(r)\lra N^*(EC_r)_{\leq m(r-1)}$ given by $e_k\mapsto (\tilde{r}!)^{m-\ell-1}(\tilde{r}-\tilde{s}-1)!\psi_k$ is a homomorphism of cochain complexes.
%\begin{remark}
...
\begin{proof}
	...
	%
	%The second statement follows because there is a summand in $\psi_k^0$ for each orbit of $\Cyc_r$ in $\psi_k$.
	%
...
\begin{proof}
	...
	%If $b>A$, as $b$ is odd, it is different from $r-1$, hence $d^{i,b}A$ is good. If $b<A$, then
	%%
	%%if $A = (a_{\sigma(0)},\ldots,a_{\sigma(k)})$ is a summand in the right (i.e., an even or odd word), then
	%%\[
	%%\rho^{b}(A) = \begin{cases}
	%%d^{\sigma(s),a_{\sigma(s)}}\left(A\smallsetminus \{a_{\sigma(s)}\}\right) & \text{if $A$ is odd and $a_{\sigma(s)} = \min A$}, \\
	%%d^{\sigma(s),a_{\sigma(s)}}\left(A\smallsetminus \{a_{\sigma(s)}\}\right) & \text{if $A$ is even and $a_{\sigma(s)} = \max A$},
	%%\end{cases}
	%%\]
	%%with $A\smallsetminus \{a_{\sigma(s)}\}\in \psi_{k-1}$ because it is even and does not contain $r-1$.

	%Moreover, if $k$ is odd, then $d^{i,b}A$ is always good.
