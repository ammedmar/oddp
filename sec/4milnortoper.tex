...
In this section we will construct a cosemi-simplicial structure on the periodic resolution $\Per{r}$ of the cyclic group $\Cyc_r$ and a chain map $\chains(\partial\asimplex^{r-1})^{\ot n+1}\to \Per{r}$ that commutes with the cofaces
\begin{align*}
	\d^i\colon \chains(\partial\asimplex^{r-1})^{\ot n+1} & \lra \sus{1-r} \chains(\partial\asimplex^{r-1})^{\ot n+2}
	\\
	\d^i\colon \Per{r} &\lra \sus{1-r}\Per{r}.
\end{align*} 
% cosemi-simplicial map $\sus{(\bullet+1)(r-1)}\chains(\partial\asimplex^{r-1})^{\ot n+1}\to \sus{(\bullet+1)(r-1)}\Per{r}$. 
...
\end{definition}
\begin{definition}
	The sequence of $R\Cyc_r$-chain complexes $[n]\mapsto \sus{(\bullet+1)(1-r)}\Per{r}$ has a cosemi-simplicial structure with coface maps
	\[
		\d^i\colon \Per{r}\to \sus{1-r}\Per{r}
	\]
	given by $\d^i(\tau) = \sus{(r-1)}\tau$	in degrees $>0$ and $\d^i(\varnothing) = \partial \sigma$.
\end{definition}
	Recall the coface maps $\d^0,\d^1\colon \chains(\partial\asimplex^{r-1})\to \sus{1-r}\chains(\partial\asimplex^{r-1})^{\ot 2}$ from \cref{def:milnor}
\begin{align*}
	\d^0(\tau) &= \partial \sigma \ot \rho^{-1}(\tau) 
	&
	\d^1(\tau) &= \tau \ot \partial \sigma,
\end{align*}
as well as the join product (\cref{def:join_product}) $\chains(\asimplex^{r-1})\otimes \chains(\asimplex^{r-1})\to \sus{1-r}\chains(\asimplex^{r-1})$.
...
%			\psi(\tau_0\otimes \ldots \otimes  \tau_n) &= \sus{(r-1)} \psi(\tau_0\otimes \ldots\otimes  \xi(\tau_{n-1}\otimes \tau_n)) & \text{if $|\tau_{n-1}| + |\tau_n| \geq r$}
	\end{align*}
	In addition, $\psi\circ \d^i = \d^i\circ \psi$ for each $i=0\ldots n$.
\end{proposition} 
\begin{proof}
...
\federico{\begin{remark}[(federico:no estoy seguro de ponerla)]
	The join product on $\chains(\asimplex^{r-1})$ induces the product on the negative part of Tate cohomology \cite{}. 
\end{remark}}

 \begin{remark}
 	Observe that because $\xi(\tau\ot \emptyset) = \xi(\emptyset\ot \tau) = \tau$, the map $\psi$ constructed factors through the Milgram resolution:
\[
...
\Per{r}
\]
where the middle chain complex has been augmented.
\end{remark}
...
%	\chains(\partial\simplex^{r-1})\ar[r]^{s_*^{\simplex}}\ar@/_2.0pc/[rr]^{s_*} & \sus{}\chains(\Psd \partial\simplex^{r-1})\ar[r]^{s^{\Psd}_*} & \chains(\sd\partial\simplex^{r-1})
%}
%\]
\[
	\xymatrix{
...
%\end{equation}
	...
	\end{equation}
Notice that the letter $b$ plays a different role in each side: on the left, it is a dual generator of the normalized cochain complex, thus it makes sense to permute its entries changing the generator by a sign. On the right, $b$ is a vertex of $\sd\partial\simplex^{r-1}$. Observe also that, with the notation above, any reordering of $b$ entails also a reordering of $a$, thus these reorderings do not change the sign of the right-hand side.
...
\begin{align*}
	\alex_{\sd} \colon \chains(\sd \partial\simplex^{r-1})& \lra \chains(\sd \partial\simplex^{r-1}),
	&
	\alex_{\Psd} \colon \chains(\Psd \partial\simplex^{r-1})& \lra \chains(\Psd \partial\simplex^{r-1}).
\end{align*}
...
%		\xymatrix{
%			\sus{-r}\chains(\partial \simplex^{r-1})\ot \chains(\partial\simplex^{r-1})\ar[r]^-{\id\ot \alex}
%			\ar[d]^{\twist} %\ar@{}[dr] | {(-1)^{n+1}}
%			&\chains(\Psd \partial \asimplex^{r-1}) \ar[r]^{s_*^{\Psd}}\ar[d]_{\alex_{\Psd}} %\ar@{}[dr] | {(-1)^{n+1}}
%			& \sus{-1}\chains(\sd \partial \asimplex^{r-1})
%			\ar[d]^{\alex_{\sd}}
%			\\
%			\sus{-r}\chains(\partial \simplex^{r-1})\ot \chains(\partial\simplex^{r-1})
%			\ar[r]^-{\id \ot \alex} 
%			&\chains(\Psd \partial \asimplex^{r-1}) 
%			\ar[r]^{s_*^{\Psd}}
%			& \sus{-1}\chains(\sd \partial \asimplex^{r-1}).
%		}
%	\end{equation*}
		\begin{equation*}
			\xymatrix{
		\susp{-r}\chains(\partial \simplex^{r-1})\ot \chains(\partial\simplex^{r-1})\ar[r]^-{\id\ot \alex}
		\ar[d]^{\twist} %\ar@{}[dr] | {(-1)^{n+1}}
		&\chains(\Psd \partial \asimplex^{r-1}) \ar[r]^{s_*^{\Psd}}\ar[d]_{\alex_{\Psd}} %\ar@{}[dr] | {(-1)^{n+1}}
		& \susp{-1}\chains(\sd \partial \asimplex^{r-1})
		\ar[d]^{\alex_{\sd}}
		\\
		\susp{-r}\chains(\partial \simplex^{r-1})\ot \chains(\partial\simplex^{r-1})
		\ar[r]^-{\id \ot \alex} 
		&\chains(\Psd \partial \asimplex^{r-1}) 
		\ar[r]^{s_*^{\Psd}}
		& \susp{-1}\chains(\sd \partial \asimplex^{r-1}).
...
\begin{definition}
	...
	\[\sd \partial \simplex^{r-1} \overset{\alex_{\sd}}{\lra} \sd \partial\simplex^{r-1} \overset{g}{\lra} \partial\simplex^{r-1}\overset{\rho^{-1}}{\lra} \partial\simplex^{r-1} \]
...
\]
because $g_*([a])$ is some vertex $v$ of $\simplex^{r-1}$, and $\partial(v) = \partial\sigma$ in $\Per{r}$. Additionally, notice that $\lambda(a,b)$ and $\lambda(b,a)$ have the same parity because $r$ is odd.\federico{Maybe we chould choose the other isomorphism for $\alex$...}
...
%	where $\pi$ is the permutation that orders the values on the right. 
	...
	The second assertion is evident. For the last one, if $\tau$ is a face of $\sd \partial \simplex^{r-1}$, then $\rho^{-1}(g(\alex(\tau))) = \rho^{-1}(x_{\tau^c})$ is the cyclic predecessor of $x_{\tau^c}$, which does not belong to $\tau^c$, and therefore belongs to $\tau$.
\end{proof}
