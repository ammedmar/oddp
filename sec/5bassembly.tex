...
%It has the following differential:
%\[\partial(b \ot a) = (-1)^{|b|}(\delta b \ot a + b \ot \partial a)\]
%\[\partial(a \ot b) = \partial a + (-1)^{|a|-|b|} \delta b\]
...
%\begin{proposition} If $f \colon C_*(\partial \simplex^n) \ot C_*(\partial \simplex^n) \lra C_*(\Sigma^n\partial \simplex^n)$ is a homomorphism satisfying the conditions, then $f$ factors as
...
\begin{proof}
	...
	\end{align*}
	%It is clear that $g$ does not vanish on this summand, because $\omega_{i+1}$ is always different from $\omega_i$. On the other hand, if we depart from this rule at the $i$th stage defining $\omega_i = \omega_{i+1}\smallsetminus \{v\}$ with $v\in \omega_{i+1}$ but $v\neq g(\omega_{i+1})$, then the image by $g$ will not contain the vertex $v$, and therefore will be degenerate.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{The cyclic subdivision map for $\partial \simplex^4$ on the face $[0,1,2,3]$ (the other faces are in the $C_r$-orbit of this face).}
	\label{fig:my_label}
	%\federico{to be discarded if it is too complicated}
\end{figure}
