...
Except for the last part of this section, we assume that $r$ is odd. In \cref{prop:chains_to_minimal} we construct a chain map from $\chains(\asimplex^{r-1})$ to $\Waug{r}$. In \cref{prop:per_to_minimal} we extend it to a chain map from $\Per{r}$ to $\Waug{r}$ that intertwines the endomorphisms $\theta$ of each of these chain complexes. In \cref{ss:milnor} we explain how the above chain map yields a chain map from the Milgram resolution $\chains(\EC_r)$ to $\Waug{r}$, and in \cref{ss:even} we discuss the case $r=2$. Recall that $\tilde{r} = \lfloor \frac{r-1}{2}\rfloor$.
...
\begin{align*}
	...
	\end{cases}
\end{align*}
Let $A= a_0\prec a_1\prec\dots\prec a_{q-1}$ be a cyclically ordered sequence in $\{0,1,\dots,r-1\}$. Say that $a_i$ is \emph{even (resp. \emph{odd}) in the sequence} if $a_{i+1}-a_i$ is an even number (resp. an odd number). Note that $a_q=a_0+r$.
...
\begin{align*}
	...
	\Phi_o(a_0,\dots,a_{q-1}) &= \begin{cases}
		...
		0 & \text{otherwise.}
	\end{cases}
\end{align*}\martin{If I want to compute $\Phi_e(4, 1, 2)$, for $r=5$, where the only even entry is $4$, do I compute $\phi_e(4,1)$ or $\phi_e(4,6)$? If it is the second, the formula should be $\phi_e(\bar{a}_j,\bar{a}_{j+1})$, right? Similar question for $\Phi_o$}

A \emph{cyclic representative} of a generator $[a_0,\dots,a_{q-1}]\in \chains(\partial \asimplex^{r-1})$ is a cyclically ordered sequence $(a_{\perm(0)},\dots,a_{\perm(q-1)})$. Removing an element from the sequence keeps the property of being cyclically ordered. The assignments $\Phi_e(A)$ and $\Phi_o(A)$ induce well defined assigments $\Phi_e[a_0,\dots,a_{q-1}]$ and $\Phi_o[a_0,\dots,a_{q-1}]$ on the generators of $\chains(\asimplex^{r-1})$. In what follows we work always with these cyclic representatives. Let $\varphi(q) = \lfloor\frac{r-q-1}{2}\rfloor$ if $q<r-1$ and $\varphi(r-1) =1$.

\begin{proposition}\label{prop:chains_to_minimal} The linear homomorphism with $\bZ[\frac{1}{\tilde{r}!}]$-coefficients
	\begin{align*}
		\Phi \colon \chains(\partial \asimplex^{r-1})& \lra \Waug{r}
		&
		\Phi(A) = \begin{cases}
			e_0 & \text{if $q=0$ and $A = \varnothing$}\\
...
\begin{proposition}\label{prop:per_to_minimal}
	The chain map $\Phi$ extends to a chain map $\Phi\colon \Per{r}\to \Waug{r}$ defining $\Phi(\sus{(r-1)m}\tau) = (\tilde{r}!)^{-m}\theta^{m}\Phi(\tau)$. As a consequence, $\theta\circ \Phi = (\tilde{r}!)\Phi\circ \theta$.	
\end{proposition}
\begin{proof}                                                                          
	Let $v = (i)$ be a vertex of $\asimplex^{r-1}$. We need to prove that $\Phi\circ \partial(\sus{r-1}v) = \partial\circ \Phi(\sus{r-1}(v))$. The right hand-side is 
	\begin{align*}
		\frac{1}{\tilde{r}!}\circ \partial\circ \theta \circ \Phi(v) &= 
		\frac{1}{\tilde{r}!} \partial\circ \theta (\frac{1}{\tilde{r}}\phi_e(i)) \\
		&= \frac{1}{\tilde{r}!} \partial\circ \theta \left(\frac{1}{\tilde{r}}\sum_{i=1}^{\frac{r-1}{2}}\rho^{j+2i+1} e_{1}\right) \\
		&= \frac{1}{\tilde{r}!} \theta\circ\partial \left(\frac{1}{\tilde{r}}\sum_{i=1}^{\frac{r-1}{2}}\rho^{j+2i+1} e_{1}\right) \\
	...
	\begin{align*}
		\Phi(\partial\sigma) &= \sum_j (-1)^j\Phi\left(0,\ldots,\hat{j},\ldots,r-1\right) \\
		&= \frac{1}{\tilde{r}!}\sum_j (-1)^j(-1)^{j}\phi_e(j-1,j+1)\cdot e_{r-1} \\
		&= \frac{1}{\tilde{r}!}\sum_j (-1)^j(-1)^{j} \rho^{j+1} e_{r-1} \\
		&= \frac{1}{\tilde{r}!}N(e_{r-1}).
	\end{align*}
\end{proof}
