...
A representative $[a_0,\dots,a_{q-1}]$ of a generator in $\chains(\partial \asimplex^{r-1})$ is a \emph{cyclically ordered representative} if the sequence $(a_0,\dots,a_{q-1})$ is $r$-cyclically ordered. Let $A=[a_0,\dots,a_{q-1}]\in \chains(\partial \asimplex^{r-1})$ be a cyclically ordered representative. Say that $a_i$ is \emph{even \emph{(resp. \emph{odd})} in $A$} if $\bar{a}_{i+1}-\bar{a}_i$ is an even number (resp. an odd number). 
...
\begin{proposition}\label{prop:per_to_minimal}
	The chain map $\Phi$ extends to a chain map $\Phi\colon \Per{r}\to \Waug{r}$ defining $\Phi(\sus{(r-1)m}\tau) = (\tilde{r}!)^{-m}\theta^{m}\Phi(\tau)$. As a consequence, $\theta\circ \Phi = (\tilde{r}!)\Phi\circ \theta$.\martin{Change $\theta$ to the current notation} 	
\end{proposition}
\begin{proof}                                                                          
